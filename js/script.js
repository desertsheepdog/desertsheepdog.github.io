...
function displayTutorial(event){
	if(event.currentTarget.parentNode.id === "tutorials"){
		if(event.currentTarget.innerHTML === "Arrays") {
			...
			$("#example").show();
			$("#test").children("p").html(arraysTest);
			...
			$("#example").show();
			$("#test").children("p").html(forLoopTest);
			$("#tutorialScript").html(forLoopScript);
			...
			$("#example").show();
			$("#test").children("p").html(whileLoopTest);
			$("#tutorialScript").html(whileLoopScript);
			...
			$("#example").show();
			$("#test").children("p").html(doWhileLoopTest);
			$("#tutorialScript").html(doWhileLoopScript);
...
var arraysTest = "This section of code will create a variable named <b>metalBands</b> and by adding <b>= [ ]</b> makes it an Array, which will have a preset length of <b>4</b>.<br><br><b><font color='green'>var</font> metalBands = [<font color='yellow'>4</font>];<br>metalBands[<font color='yellow'>0</font>] = <font color='orange'><q>In Flames</q></font>;<br>metalBands[<font color='yellow'>1</font>] = <font color='orange'><q>Trivium</q></font>;<br>metalBands[<font color='yellow'>2</font>] = <font color='orange'><q>Amon Amarth</q></font>;<br>metalBands[<font color='yellow'>3</font>] = <font color='orange'><q>Children of Bodom</q></font>;<br>document.getElementById(<font color='orange'><q>block1</q></font>).innerHTML = metalBands[<font color='yellow'>0</font>];<br>document.getElementById(<font color='orange'><q>block2</q></font>).innerHTML = metalBands[<font color='yellow'>1</font>];<br>document.getElementById(<font color='orange'><q>block3</q></font>).innerHTML = metalBands[<font color='yellow'>2</font>];<br>document.getElementById(<font color='orange'><q>block4</q></font>).innerHTML = metalBands[<font color='yellow'>3</font>];</b><br><br>Since Arrays beginning data set starts at 0 the first item we will store will be in <b>metalBands[0]</b> this will also be used when retreiving the data from that specific location.<br>Below is the results of this code:"

var arraysScript = "<script>var metalBands = [];metalBands[0] = 'In Flames';metalBands[1] = 'Trivium';metalBands[2] = 'Amon Amarth';metalBands[3] = 'Children of Bodom';document.getElementById('block1').innerHTML = metalBands[0];document.getElementById('block2').innerHTML = metalBands[1];document.getElementById('block3').innerHTML = metalBands[2];document.getElementById('block4').innerHTML = metalBands[3];</script>"
...
var forLoopTest = "For Loop with the Array from the Arrays tutorial.<br><br><b><font color='green'>var</font> metalBands[<font color='orange'><q>In Flames</q></font>, <font color='orange'><q>Trivium</q></font>, <font color='orange'><q>Amon Amarth</q></font>, <font color='orange'><q>Children of Bodom</q></font>]<br><font color='green'>for</font> (<font color='green'>var</font> i = <font color='yellow'>0</font>; i < metalBand.length; i++){<br>   document.getElementById(<font color='orange'><q>block</q></font> + (i + <font color='yellow'>1</font>)).innerHTML =metalBands[i];<br>}</b><br><br>Here we have the variable <b>i</b> set to 0 and the looping condition set to stop when <b>i</b> becomes greater then the total length of the array represented as <b>metalBands.length</b>.<br>Inside the For Loop we use the same DOM selector statement with <b><q>block</q> + (i + 1)</b>. since block is the root word for each div id we place it inside quotations and add <b>i</b>, but since the div ids start at 1 we add <b>+ 1</b> to conpensate for starting at 0.<br>Even though iterating 4 lines of code to do the same thing as this For Loop, it's extremely easy to add on, and when working with large numbers of iteration for loops can help reduce the size of overall code.<br>Below is the result of this code:";

var forLoopScript = "<script>var metalBands = ['In Flames', 'Trivium', 'Amon Amarth', 'Children of Bodom'];for(var i = 0; i < metalBands.length; i++){document.getElementById('block' + (i + 1)).innerHTML = metalBands[i];}</script>"
...
var whileLoopTest = "<br><b><font color='green'>var</font> metalBands[<font color='orange'><q>In Flames</q></font>, <font color='orange'><q>Trivium</q></font>, <font color='orange'><q>Amon Amarth</q></font>, <font color='orange'><q>Children of Bodom</q></font>];<br><font color='green'>var</font> i = <font color='yellow'>0</font>;<br><font color='green'>while</font> (i &lt; metalBands.length) {<br>   document.getElementById(<font color='orange'><q>block</q></font> + (i + <font color='yellow'>1</font>)).innerHTML =metalBands[i];<br>   i++;<br>}</b><br><br>This While Loop utilizes the same <b>metalBands</b> Array and creates a variable <b>i</b> and set to 0. the condition is that the loop will repeat as long as <b>i</b> is less than the total length of the array. Before the closing braket <b>i++</b> is added to increment the counter variable and prevent an infinite loop.";

var whileLoopScript = "<script>var metalBands = ['In Flames', 'Trivium', 'Amon Amarth', 'Children of Bodom'];var i = 0;while(i < metalBands.length){document.getElementById('block' + (i + 1)).innerHTML = metalBands[i];i++;}</script>";
...
var doWhileLoopTest = "<br><b><font color='green'>var</font> metalBands[<font color='orange'><q>In Flames</q></font>, <font color='orange'><q>Trivium</q></font>, <font color='orange'><q>Amon Amarth</q></font>, <font color='orange'><q>Children of Bodom</q></font>];<br><font color='green'>var</font> i = <font color='yellow'>0</font>;<br><font color='green'>do</font>{<br>   document.getElementById(<font color='orange'><q>block</q></font> + (i + <font color='yellow'>1</font>)).innerHTML =metalBands[i];<br>   i++;<br>}<font color='green'>while</font> (i &lt; metalBands.length);</b><br><br>This looks almost the same as our While Loop, except the order has changed and the inclusion of the <b>do</b> header at the beginning. Whether the statement was true or false the the first iteration would take place and the <b><q>block1</q></b> div will always display <b><q>In Flames</q></b>.";

var doWhileLoopScript = "<script>var metalBands = ['In Flames', 'Trivium', 'Amon Amarth', 'Children of Bodom'];var i = 0;do{document.getElementById('block' + (i + 1)).innerHTML = metalBands[i];i++;}while(i < metalBands.length);</script>";
